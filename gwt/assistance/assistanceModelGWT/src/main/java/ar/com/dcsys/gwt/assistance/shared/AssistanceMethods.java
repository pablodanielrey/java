package ar.com.dcsys.gwt.assistance.shared;

public class AssistanceMethods {

	public static final String findIdsPersonsWithPeriodAssignation = "findIdsPersonsWithPeriodAssignation";
	
	public static final String getJustifications = "getJustifications";
	public static final String persistJustification = "persistJustification";
	public static final String removeJustification = "removeJustification";
	public static final String justify = "justify";
	public static final String findByPerson = "findByPerson";
	public static final String findByPersonValue = "findByPersonValue";
	public static final String removeJustificationDate = "removeJustificationDate";
	public static final String persistGeneralJustificationDate = "persistGeneralJustificationDate";
	public static final String findGeneralJustificationDateBy = "findGeneralJustificationDateBy";
	public static final String removeGeneralJustificationDate = "removeGeneralJustificationDate";
	
}
