...
import ar.com.dcsys.pr.model.Method;
import ar.com.dcsys.pr.model.Param;
...
//							sb.append("\n").append(ss).append(ss).append(ss).append("eAutoBean").append(".as().setValue(").append("e").append(");");
				...
				*/
				...
				String receiverName = method.receiver.getReceiverContainerName();
				String receiverType = method.receiver.getReceiverType();
				
				sb.append("\n").append(ss).append(ss).append(ss).append(ss).append("AutoBean<").append(receiverType).append("> rec2 = AutoBeanCodex.decode(").append(manager.factory.getName()).append(",").append(receiverName).append(".class,msg);");
				sb.append("\n").append(ss).append(ss).append(ss).append(ss).append("try {");
				sb.append("\n").append(ss).append(ss).append(ss).append(ss).append(ss).append("rec.onSuccess(rec2.as().getValue());");
				sb.append("\n").append(ss).append(ss).append(ss).append(ss).append("} catch (Exception e) { }");
	...
	private void generateReceiverContainers(List<Manager> managers) {
		...
		for (Manager manager : managers) {
			for (Method m : manager.methods) {
				...
				String receiverType = rec.getReceiverType();
...
//				sb.append("\nimport ").append(stype).append(";");
				...
				sb.append("public interface ").append(rec.getReceiverContainerName()).append(" extends TypeContainer<").append(receiverType).append("> {");
				sb.append("\n").append("}");
				
				
				try {
					JavaFileObject jfo = processingEnv.getFiler().createSourceFile(manager.factory.getPackageName() + "." + rec.getReceiverContainerName());
					PrintWriter out = new PrintWriter(jfo.openWriter());
					out.println(sb.toString());
					out.flush();
					out.close();
	...
	private void generateFactories(List<Manager> managers) {
		...
		for (Manager manager : managers) {
			...
			for (Method m : manager.methods) {
				Param rec = m.receiver;
				String stype = rec.getReceiverType();
				...
				String rname = rec.getReceiverContainerName();
				String pack = rec.getPackage();
				String fullName = pack + "." + rname;
				
				sb.append("\n");
				sb.append("\n").append(ms).append("public AutoBean<").append(fullName).append("> ").append("get").append(fullName.replace(".", "_")).append("();");
				sb.append("\n").append(ms).append("public AutoBean<").append(fullName).append("> ").append("get").append(fullName.replace(".", "_")).append("(").append(rname).append(" ").append("l);");
