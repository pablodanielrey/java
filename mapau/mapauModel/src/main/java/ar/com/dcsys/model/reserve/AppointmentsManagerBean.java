...
public class AppointmentsManagerBean implements AppointmentsManager {
	...
	@Inject
	public AppointmentsManagerBean(ReservesManager reservesManager, ReserveAttemptDatesManager reserveAttemptDatesManager, UntouchableSubjectsManager untouchableSubjectsManager, 
										ClassRoomsManager classRoomsManager, PersonsManager personsManager, CoursesManager coursesManager) {
	...
	public void createNewAppointments(List<AppointmentV2> appointments) throws MapauException {
		...
		// creo el reserveattmpt y los reserveattemptdate
			...
			for (AppointmentV2 ap : appointments) {
				...
				reserveAttemptDatesManager.persist(rad);
			}
	...
	public void deleteAppointment(AppointmentV2 app) throws MapauException {
		...
		reserveAttemptDatesManager.remove(rad);
	}
	...
	public void modify(AppointmentV2 a) throws MapauException, PersonException {
		...
		String newId = reserveAttemptDatesManager.persist(modifiedRad);
	...
	public void createReserve(Appointment a, List<ClassRoom> classRooms) throws MapauException, PersonException {
		...
		ReserveAttemptDate rad = reserveAttemptDatesManager.findById(radId);
		if (rad == null) {
	...
	public List<ReserveAttemptDate> findAllAppointmentsBy(ReserveAttemptDate app, List<Date> dates, boolean checkHour) throws MapauException {
		...
		if (checkHour) {				
		 ...
		 List<ReserveAttemptDate> rads2 = reserveAttemptDatesManager.findAll(app,rdates);
	...
	public List<ClassRoom> findAllClassRoomsAvailableIn(List<ReserveAttemptDate> rads) throws MapauException {
		...
		try {
			...
			List<ReserveAttemptDate> collidingRads = reserveAttemptDatesManager.findAllCollidingWith(rdates);
	...
	public List<ReserveAttemptDate> findReserveAttemptDateBy(Date start, Date end, List<TransferFilter> filters) throws MapauException {
		...
		if (filters == null || filters.size() <= 0) {
			return reserveAttemptDatesManager.findBy(start, end);
		}
		
		List<ReserveAttemptDate> rads = reserveAttemptDatesManager.findBy(start, end);
		if (rads == null || rads.size() <= 0) {
			return rads;
		}
