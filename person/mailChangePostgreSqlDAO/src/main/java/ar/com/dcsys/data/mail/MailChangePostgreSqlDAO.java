...
public class MailChangePostgreSqlDAO implements MailChangeDAO {
	...
	@Inject
	public MailChangePostgreSqlDAO(PostgresSqlConnectionProvider cp) {
		this.cp = cp;
	}
	...
	public List<MailChange> findByMail(String mail) throws PersonException {
		...
		try {
			Connection con = cp.getConnection();
			try {
				String query = "select * from persons_mailChanges where mail = ?";
				PreparedStatement st = con.prepareStatement(query);
				try {
					st.setString(1, mail);
					ResultSet rs = st.executeQuery();
					try {
						List<MailChange> result = new ArrayList<>();
						while (rs.next()) {
							MailChange mc = getMailChange(rs);
							result.add(mc);
						}
